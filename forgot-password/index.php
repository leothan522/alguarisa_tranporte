                    ...
                    <div class="row g-0">
                        <div class="col-lg-6">
                            <div class="card-body p-md-5 mx-md-4">
                                ...
                                <form class="position-relative" id="form_forgot_password" novalidate>

                                    <div class="mb-4">
                                        <p class="fs-6 d-flex" style="text-align: justify !important;">
                                            <small class="text-muted" id="texto_mostrar">¿Olvidaste tu contraseña? Aquí puede recuperar fácilmente una nueva contraseña.</small>
                                        </p>
                                    </div>
                                    ...
                                    <div class="text-center pt-1 pb-1 d-grid gap-2" id="boton_a_login">
                                        <input type="hidden" name="opcion" value="forgot_password">
                                        <button type="submit" class="btn shadow text-white btn-block  gradient-custom-2">Solicitar nueva contraseña</button>
                                    </div>
...
<script src="<?php asset('forgot-password/_app/app.js', true); ?>"></script>
<script type="application/javascript">

    const form = document.querySelector('#form_forgot_password');
    const email = document.querySelector('#email');
    const textoMostrar = document.querySelector('#texto_mostrar');
    const link = document.querySelector('#remove_link');
    const boton = document.querySelector('#boton_a_login');
    ...
    form.addEventListener('submit', event => {
        ...
        if (form.checkValidity()) {
            ...
            const url = "_request/ForgotPasswordRequest.php";
            ajaxRequest({ url: url, form: form }, function (data) {
                if (data.result){
                    textoMostrar.classList.remove('text-muted');
                    textoMostrar.classList.add('text-success');
                    textoMostrar.textContent = data.message;
                    link.classList.add('d-none');
                    boton.innerHTML = '<a href="../login" class="btn shadow text-white btn-block  gradient-custom-2">Ir al Login</a>';
                }else {
